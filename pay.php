<?php 		
		...
		$sql="Select software.pid, pname, Price FROM software,cart where cart.pid=software.pid and cart.uid=$uid";
		echo $sql;
		foreach($db->query($sql) as $row)
		{
			...
			echo "<br>$pid";
			$sql1 = "insert into payment(tid,date,price,pid,uid) values (?,?,?,?,?)";
			$db1 = new PDO(DB_INFO,DB_USER,DB_PASS);
			
			$stmt=$db1->prepare($sql1);
			$stmt->execute(array($tid,$date,$price,$pid,$uid));
			$stmt->closecursor();
...
<div class="main">
    ...
    <span><input type="text" id="key" name="key" placeholder="Merchant Key" value="EDPwKipF" /></span>
    </div>
    
    <div class="dv">
    <span class="text"><label>Merchant Salt:</label></span>
    <span><input type="text" id="salt" name="salt" placeholder="Merchant Salt" value="LOfBeejMB9" /></span>
    </div>
    ...
    <span><input type="text" id="amount" name="Price" placeholder="Price" value="<?php echo  $price; ?>" /></span>    
    </div>
